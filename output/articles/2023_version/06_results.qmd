...
data_model <- readRDS("../../../input/proc_data/05_student_proc_2023.rds")
...
#| fig-cap: "General and specialized ICT self-efficacy. Data: ICILS Chile 2023"

selfeff_items <- data_model|>
  select(
...
relative_freq_selfeff <- rbind(
  selfeff_items |>
    drop_na(is3g24a) |>
    count(variable = is3g24a) |>
    mutate(prop = n / sum(n), name = "Edit graphic images",
           type = "General Self-efficacy"),
  
  selfeff_items |>
    drop_na(is3g24b) |>
    count(variable = is3g24b) |>
    mutate(prop = n / sum(n), name = "Write or edit text",
           type = "General Self-efficacy"),
  
  selfeff_items |>
    drop_na(is3g24c) |>
    count(variable = is3g24c) |>
    mutate(prop = n / sum(n), name = "Search and find information on internet",
           type = "General Self-efficacy"),
  
  selfeff_items |>
    drop_na(is3g24e) |>
    count(variable = is3g24e) |>
    mutate(prop = n / sum(n), name = "Change a device settings",
           type = "General Self-efficacy"),
  
  selfeff_items |>
    drop_na(is3g24f) |>
    count(variable = is3g24f) |>
    mutate(prop = n / sum(n), name = "Create a multi-media presentation",
           type = "General Self-efficacy"),
  
  selfeff_items |>
    drop_na(is3g24g) |>
    count(variable = is3g24g) |>
    mutate(prop = n / sum(n), name = "Upload content to an online profile",
           type = "General Self-efficacy"),
  
  selfeff_items |>
    drop_na(is3g24h) |>
    count(variable = is3g24h) |>
    mutate(prop = n / sum(n), name = "Insert an image into a document",
           type = "General Self-efficacy"),
  
  selfeff_items |>
    drop_na(is3g24i) |>
    count(variable = is3g24i) |>
    mutate(prop = n / sum(n), name = "Install a program",
           type = "General Self-efficacy"),
  
  selfeff_items |>
    drop_na(is3g24j) |>
    count(variable = is3g24j) |>
    mutate(prop = n / sum(n), name = "Judge information find on the internet",
           type = "General Self-efficacy"),
  
  selfeff_items |>
    drop_na(is3g24m) |>
    count(variable = is3g24m) |>
    mutate(prop = n / sum(n), name = "Find original source of an article in the internet",
           type = "General Self-efficacy"),
  
  selfeff_items |>
    drop_na(is3g24d) |>
    count(variable = is3g24d) |>
    mutate(prop = n / sum(n), name = "Build a webpage",
           type = "Specialized Self-efficacy"),
  
  selfeff_items |>
    drop_na(is3g24k) |>
    count(variable = is3g24k) |>
    mutate(prop = n / sum(n), name = "Use a text-based programming language",
           type = "Specialized Self-efficacy"),
  
    selfeff_items |>
    drop_na(is3g24l) |>
    count(variable = is3g24l) |>
    mutate(prop = n / sum(n), name = "Use visual coding",
           type = "Specialized Self-efficacy")
)
...
relative_freq_selfeff <- relative_freq_selfeff |>
  mutate(
    name = factor(name, levels = c(
      ...
      "Install a program",
      "Upload content to an online profile",
      "Edit graphic images",
      "Judge information find on the internet",
      "Create a multi-media presentation",
      "Change a device settings",
      "Find original source of an article in the internet",
      "Build a webpage",
    ...
    "3" = "Very well",
    "2" = "Moderately very well",
    "1" = "I have never done this, but I could work out how to do this",
    "0" = "I do not think I could do this"),
