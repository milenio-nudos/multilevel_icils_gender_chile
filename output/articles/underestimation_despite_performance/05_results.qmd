...
Null multilevel model for general self-efficacy revealed an Intraclass Correlation Coefficient[^icc] of 0.05, and 0.06 for specialized self-efficacy. 5% and 6% of the total variance in the dependent variables can be attributed to between-school differences. The remaining 95% or 96% of the variance is within-school variance, suggesting substantial variability at the student level. But since there is proportion of the variances associated with between schools differences, the decision to explore multilevel models is optimal.

[^icc]: ICC for now.

For reasons of parsimony, it was decided to work with fixed effects models rather than random effects. Verisimilitude tests were designed to compare fit between fixed-effects and random-effects models. The results show non significant differences between models and fixed-effects models fit a little better with data distribution (see @tbl-anova-general-test and @tbl-anova-specialized-test). Although there is a clear hierarchical structure of the data (which suggest to use random-effects models), low number of levels are explored (only two levels) and the number of cases nested at level two are few (none is greater than 20).

@tbl-general-multilevel and @tbl-specialized-multilevel displays the results of the multilevel regression models hierarchically, adding first student level variables and interactions, then school level variables and moderations[^modelnote]. For gender, the reference category is Male, and for gender composition is Mixed schools.

[^modelnote]: Control variables were omitted in the tables, but the estimation consider them.

**General ICT Self efficacy**

When the model have sex as the unique independent variable, in mean, girls score 0.75 points more than boys in the general self-efficacy index. This is a lower magnitude (less than one point), but statistically significant at a 95% level of confidence. When other factors as CIL score or school level characteristics enter as control variables, significant gender differences are lost. In this way, gender differences are not intrinsic, as can be explained by other elements of the analysis.

As the opposite case, CIL score have a positive effect which maintains it statistical significance (at greater than 99.9% confidence level) controlling for all other variables used. The beta of scaled CIL score round between 0.34 and 0.37 around models.

Model 3 suggest non-significant results in interaction between sex and CIL score of the student. Model 4 and 7 determines Gender composition have non-significant differences between schools. Controlling by all variables, Model 6 indicate when CIL of the school increase in one point, the general self-efficacy of the student decrease in more than 0.10. In other words, students rounded with pairs with more computer literacy will feel less capabilities with simple tasks using ICT. School CIL and gender moderation is non-significant.

The explained variance of all models was estimated from two different methods. In both estimates, the explained part is less than 15%. Other elements need to be integrated to explain behaviour of general ICT self-efficacy.
...
About specialized self efficacy regressions, when gender is introduce as first independent variable, girl have 1.76 less coefficient, with a confidence level major than 99.9%. If other control variables are introduced, this result keeps with lower changes (less when it’s added an interaction effect between gender and CIL or a moderation between gender and schoo CIL). It could be said there’s advantages from boys over girls in specialized ICT self-efficacy that are intrinsic to gender condition of the students.

Counter-intuitively, the score on CIL test have a significant negative effect of 0.09 when it’s added only with sex as independent variable. But as interaction effect or school level variables enter into the equation, the coefficient decrease and lose it significance. So, against expectations, students with more computer literacy will tend to have less expectations of self-efficacy in advanced tasks using ICT, but this finding is not consistent controlling by elements related with school composition.

About school level variables, once again, women proportion of students didn’t show significant results. But last models show the student individual specialized self-efficacy decrease by 0.22 to 0.26 when the school increase CIL score aggregated mean by one point (p<0.001). Students have less expectations of self-efficacy in advanced tasks using ICT as their pairs have more Computer and Literacy. This effect have non-significant moderation with the type of school gender composition.

The explained variance of all models in the table is lower than 10%. It’s necessary to keep exploring other hypothesis to reveal the reasons of the specialized ICT self-efficacy distribution.
